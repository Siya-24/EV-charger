package com.example.evchargingapp.stats

import android.os.Bundle
import android.os.StrictMode
import android.widget.TextView
import androidx.appcompat.app.AppCompatActivity
import com.example.evchargingapp.BuildConfig
...
class StatsActivity : AppCompatActivity() {
    ...
    private fun calculateAndDisplayStats() {
        ...
        val formattedDuration = String.Companion.format(Locale.getDefault(), "%.0f hr %.0f min", durationHours.toLong().toDouble(), durationMinutes.toDouble())
        ...
        avgPowerTextView.text = String.Companion.format(Locale.getDefault(), "%.2f kW (avg)", chargerOutputPowerKw)

        // 3. Energy Consumed (kWh) - based on charger output power
        val energyConsumedKwh = chargerOutputPowerKw * durationHours
        energyConsumedTextView.text = String.Companion.format(Locale.getDefault(), "%.2f kWh", energyConsumedKwh)

        // 4. Cost of Charging
        val cost = energyConsumedKwh * electricityRatePerKWh
        costTextView.text = String.Companion.format(Locale.getDefault(), "$%.2f", cost) // Adjust currency symbol as needed

        // 5. Battery Percentage Gained
        val percentageGained = endBatteryPercent - startBatteryPercent
        batteryGainedTextView.text = String.Companion.format(Locale.getDefault(), "%d %%", percentageGained)
        ...
        chargingSpeedTextView.text = String.Companion.format(Locale.getDefault(), "%.1f %s/hr", rangeGainedPerUnit, rangeUnit)
        ...
        approxTimeTextView.text = String.Companion.format(Locale.getDefault(), "%d hr %d min (to %d%%)", hoursRemaining, minutesRemaining, targetBatteryPercent)
    }
}